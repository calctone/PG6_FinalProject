...
#import "ColorCircleViewController.h"  //ADDED BCA
...
#define FOLLOW @"FOLLOW"  //ADDED BCA
...
- (void)perform:(NSString *)msg :(GCDAsyncSocket*) socket
{
    ...
    else if ([msg caseInsensitiveCompare:FOLLOW] == NSOrderedSame) //Added BCA
    {
      
        [self.Romo3 tiltToAngle:70 completion:0];
        _min = 75;
        _max = 130;
        if (circles.size() > 0)
        {
            [self.Romo3 driveForwardWithSpeed:.25];
        }
        else
        {
